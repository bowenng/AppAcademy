...
def harrison_ford
  ...
  Actor.joins(:movies).joins(:castings).select("DISTINCT movies.id, movies.title").where("actors.name = ?", "Harrison Ford").where("castings.ord != 1")
end
...
def biggest_cast
  ...
  Movie.joins(:castings).select("movies.id, movies.title").group("movies.id").order("COUNT(*) DESC").limit(3)
end
