...
def below_average_years
  ...
  Movie.select('COUNT(*) AS bad_movies, yr').group(:yr).order('COUNT(*)').where('movies.score < 5')
end
...
def uma_movies
  ...
  Movie.joins(:actors).select(:id, :title, :yr).where('actors.name = ?', 'Uma Thurman').order('movies.yr ASC')
end
