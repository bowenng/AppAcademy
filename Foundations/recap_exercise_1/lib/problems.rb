...
def all_vowel_pairs(words)
    ...
    words.each_with_index do |w1,i|
        words.each_with_index do |w2,j|
            if i < j
                pair = (w1 + " " + w2)
                all_vowels << pair if vowels.all? {|vol| pair.include? vol}
            end
        end
    end
    all_vowels
...
def find_bigrams(str, bigrams)
    contained = []
    bigrams.each{|bigram| contained << bigram if str.include? bigram}
    contained
end
...
class String
    ...
    def caesar_cipher(num)
        ...
        shifted = self.chars.map {|char| alphabet[(alphabet.index(char)+num) % 26]}
        shifted.join("")
    end
end
